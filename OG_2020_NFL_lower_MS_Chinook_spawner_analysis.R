...
location<-c("NFLewis")
species<-c("Chinook")
JS_stats_ALL<-readRDS(glue("data/Lewis_2020_JS_data.rds"))
...
# cols<-brewer.pal(length(MR_data$uTot),"Blues")
# par(mfrow=c(1,1))
# x<-matrix(rep(1:MR_data$s, length(MR_data$uTot)),ncol=length(MR_data$uTot))
# #par(mfrow=c(2,1),mar=c(4,4,4,4))
# matplot(out$BUGSoutput$median$p,type="l",col=cols[1:length(MR_data$uTot)],lty=1,lwd=2,ylim=c(0,1),xlim=c(1,MR_data$s), ylab="Capture")
# legend("topright",legend=names(MR_data$uTot),lty=rep(1,length(MR_data$uTot)),lwd=2,col=cols[1:length(MR_data$uTot)])
# #matplot(y=MR_data$m/MR_data$T,x=x,type="l",col=cols[1:length(MR_data$uTot)],lty=2,lwd=2,ylim=c(0,1),xlim=c(1,MR_data$s),ylab ="RAW (m/T)")
# #legend("topright",legend=names(MR_data$uTot),lty=rep(2,length(MR_data$uTot)),lwd=2,col=cols[1:length(MR_data$uTot)])
# #par(mfrow=c(2,1),mar=c(4,4,4,4))
# matplot(out$BUGSoutput$median$phi,type="l",col=cols[1:length(MR_data$uTot)],lty=1,lwd=2,ylim=c(0,1),xlim=c(1,c(MR_data$s-1)),ylab="Survival")
# legend("topright",legend=names(MR_data$uTot),lty=rep(1,length(MR_data$uTot)),lwd=2,col=cols[1:length(MR_data$uTot)])
# #matplot(y=MR_data$r/MR_data$R,x=x,type="l",col=cols[1:length(MR_data$uTot)],lty=2,lwd=2,ylim=c(0,1),xlim=c(1,c(MR_data$s-1)),ylab ="RAW (r/R)")
# #legend("topright",legend=names(MR_data$uTot),lty=rep(2,length(MR_data$uTot)),lwd=2,col=cols[1:length(MR_data$uTot)])
# #par(mfrow=c(1,1))
# matplot(out$BUGSoutput$median$pent,type="l",col=cols[1:length(MR_data$uTot)],lty=1,lwd=2,ylim=c(0,1),ylab="Pent")
# legend("topright",legend=names(MR_data$uTot),lty=1,lwd=2,col=cols[1:length(MR_data$uTot)])
# dev.off()
